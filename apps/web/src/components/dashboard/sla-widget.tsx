...
const SEVERITY_COLORS: Record<string, { bg: string; text: string; dot: string }> = {
  ...
  INFO: { bg: 'bg-gray-50', text: 'text-gray-700', dot: 'bg-gray-400' },
};
  ...
  return (
    <div className="rounded-lg border border-gray-200 bg-white p-6">
      ...
      <div className="mt-4 grid grid-cols-2 gap-3">
        <div
          className={`rounded-lg p-4 ${hasBreaches ? 'bg-red-50 border border-red-200' : 'bg-green-50 border border-green-200'}`}
        >
          <p className={`text-2xl font-bold ${hasBreaches ? 'text-red-700' : 'text-green-700'}`}>
            {data.totalBreached}
          </p>
          <p className={`text-xs font-medium ${hasBreaches ? 'text-red-600' : 'text-green-600'}`}>
            SLA Breaches
          </p>
        </div>
        <div
          className={`rounded-lg p-4 ${hasApproaching ? 'bg-yellow-50 border border-yellow-200' : 'bg-green-50 border border-green-200'}`}
        >
          <p
            className={`text-2xl font-bold ${hasApproaching ? 'text-yellow-700' : 'text-green-700'}`}
          >
            {data.totalApproaching}
          </p>
          <p
            className={`text-xs font-medium ${hasApproaching ? 'text-yellow-600' : 'text-green-600'}`}
          >
            Approaching
          </p>
        </div>
              ...
              (severity) => {
                ...
                return (
                  <Link
                    ...
                    className="flex items-center justify-between rounded-md bg-yellow-50 px-3 py-2 hover:opacity-80 transition-opacity"
                  >
                    <div className="flex items-center gap-2">
                      <span className={`h-2 w-2 rounded-full ${colors.dot}`} />
      ...
      {!hasBreaches && !hasApproaching && (
        <div className="mt-4 rounded-md bg-green-50 p-4 text-center">
          <p className="text-sm font-medium text-green-700">All cases within SLA</p>
          <p className="text-xs text-green-600">No breaches or approaching deadlines</p>
        </div>
      )}
    </div>
  );
